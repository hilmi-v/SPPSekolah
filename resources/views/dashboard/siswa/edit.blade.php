<x-layout>
    ...
    <div>
        <p class="text-2xl">edit siswa:</p>
    </div>

    <div class="mt-3 rounded-xl bg-gray-200 p-4">
        <form action="{{ route('update-siswa', ['siswa' => $siswa->nisn]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="my-5">
            ...
            <div class="my-5">
                ...
                <select name="id_kelas" id="id_kelas" class="w-full rounded-lg border-0 shadow-sm ring-0" required>
                    @foreach ($kelass as $kelas)
                        <option value="{{ $kelas->id_kelas }}" @selected(old('id_kelas', $siswa->id_kelas) == $kelas->id_kelas)>
            ...
            <div class="my-5">
                ...
                <select name="id_spp" id="id_spp" class="w-full rounded-lg border-0 shadow-sm ring-0" required>
                    @foreach ($spps as $spp)
                        <option value="{{ $spp->id_spp }}" @selected(old('id_spp', $siswa->id_spp) == $spp->id_spp)>
            ...
            <div class="my-5">
                ...
                <input type="tel" id="no_telp" class="w-full rounded-lg border-0 shadow-sm ring-0" required
                    name="no_telp" value="{{ old('no_telp', $siswa->no_telp) }}">
                @error('no_telp')
                    <p class="text-red-600">{{ $message }}</p>
                @enderror
            </div>
